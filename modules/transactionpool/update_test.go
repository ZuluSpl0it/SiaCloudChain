...
func TestValidRevertedTransaction(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		txns, err := tpt.wallet.SendSiacoins(types.ScPrimecoinPrecision.Mul64(1000), types.UnlockHash{})
		if err != nil {
			t.Fatal(err)
		}
...
// prunes transactions older than MaxTransactionAge.
func TestTransactionPoolPruning(t *testing.T) {
	...
	for i := types.BlockHeight(0); i < MaxTransactionAge+1; i++ {
		_, err = tpt2.miner.AddBlock()
		if err != nil {
			t.Fatal(err)
		}
